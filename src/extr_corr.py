...
class ExtremeCorrection:
    ...
    def __init__(
        ...
        ### Historical data
        self.max_data = (
            self.data_hist.groupby(self.yyyy_var, as_index=False)[self.var]
            .max()[self.var]
            .values
        )  # Annual Maxima
        self.max_idx = (
            self.data_hist.groupby(self.yyyy_var)[self.var].idxmax().values
        )  # Annual Maxima indices
        self.max_data_sorted = np.sort(self.max_data)  # Sorted Annual Maxima
        self.pit_data = self.data_hist[
            self.var
        ].values  # Point-in-time data (hourly, daily...)
        self.pit_data_sorted = np.sort(
            self.pit_data
        )  # Sorted point-in-time data (hourly, daily...)
        ...
        self.time_interval_hist = (
            self.n_pit / self.freq
        )  # Time interval: Number of observations / Frequency

        ### Simulated Data
        # Annual maxima
        # self.data_sim['year'] = self.data_sim[self.time_var].dt.year
        self.sim_max_data = (
            self.data_sim.groupby(self.yyyy_var, as_index=False)[self.var]
            .max()[self.var]
            .values
        )  # Simulated annual maxima data
        self.sim_max_idx = (
            self.data_sim.groupby(self.yyyy_var)[self.var].idxmax().values
        )  # Simulated annual maxima indices
        self.sim_max_data_sorted = np.sort(
            self.sim_max_data
        )  # Sorted simulated annual maxima
        self.sim_pit_data = self.data_sim[
            self.var
        ].values  # Simulated point-in-time data
        self.sim_pit_data_sorted = np.sort(
            self.sim_pit_data
        )  # Sorted simulated point-in-time data

        # Number of simulated years and point-in-time values
        self.n_sim_year_peaks = self.sim_max_data.shape[0]  # Nº of simulated years
        self.n_sim_pit = self.sim_pit_data.shape[
            0
        ]  # Nº of simulated point-in-time observations

        # Simulated time interval
        # self.time_interval_sim = self.data_sim[self.yyyy_var].max() - self.data_sim[self.yyyy_var].min()
        self.time_interval_sim = (
            self.n_sim_pit / self.freq
        )  # Time interval: Number of observations / Frequency

        # TODO: CHECK IF THIS IS CURRENTLY NECESARY
        # Divide data in intervals of nº of historical years
        self.sim_first_year = np.min(
            self.data_sim[self.yyyy_var]
        )  # First year of the simulation
        self.n_year_intervals = (
            self.n_sim_year_peaks // self.n_year_peaks
        )  # Nº of intervals to divide the simulated data
        self.sim_max_data_idx_intervals = {}  # Annual maximas per intervals
        for i_year in range(self.n_year_intervals):
            self.sim_max_data_idx_intervals[i_year] = (
                self.data_sim[
                    (
                        self.sim_first_year + self.n_year_peaks * i_year
                        <= self.data_sim[self.yyyy_var]
                    )
                    & (
                        self.data_sim[self.yyyy_var]
                        < self.sim_first_year + self.n_year_peaks * (i_year + 1)
                    )
                ]
                .groupby(self.yyyy_var)[self.var]
                .idxmax()
                .values
            )
    ...
    def _validate_config(self) -> None:
        ...
        for key, expected_type in required_fields.items():
            ...
            if not isinstance(self.config[key], expected_type):
                raise TypeError(
                    f"Configuration error: Key '{key}' must be of type {expected_type.__name__}."
                )

        # Optional fields with defaults
        optional_fields = {"mm_var": "mm", "dd_var": "dd", "folder": None}
    ...
    def _define_method(self, tolerance: float = None) -> None:
        """
        DEPRECATED
        ...
        self.poiss_parameter = (
            self.n_pot / self.time_interval_hist
        )  # Poisson parameter of historical dataset

        if tolerance is None:
            tolerance = self.poiss_parameter / 100

        # POT of simulated data
        self.sim_pot_data, self.sim_pot_data_sorted, self.sim_pot_data_locs = (
            self.obtain_pots(
                self.sim_pit_data,
        ...
        self.sim_poiss_parameter = (
            self.n_pot_sim / self.time_interval_sim
        )  # Poisson parameter of historical dataset
    ...
    def obtain_pots(
        ...
        opt_thres.threshold_peak_extraction(
            threshold=threshold, n0=n0, min_peak_distance=min_peak_distance
        )
        ...
        if optimize_threshold:
            ...
            opt_thres.threshold_peak_extraction(
                threshold=self.opt_threshold, n0=n0, min_peak_distance=min_peak_distance
            )
    ...
    def apply_correction(self, fit_diag: bool = False):
        """
    ...
    def extreme_fit(self):
        ...
        if self.method == "pot":
            shape_gpd, loc_gpd, scale_gpd = stats.genpareto.fit(
                self.pot_data - self.opt_threshold, floc=0
            )
    ...
    def plot_diagnostic(self, save: bool = True):
        """
    ...
    def gev_qqplot(self):
        ...
        probabilities = (np.arange(1, self.n_year_peaks + 1)) / (
            self.n_year_peaks + 1
        )  # Probabilidades de los cuantiles empíricos
        gev_quantiles = stats.genextreme.ppf(
            probabilities,
            c=self.parameters[2],
    ...
    def gev_ppplot(self):
        ...
        gev_probs = stats.genextreme.cdf(
    ...
    def gpd_diag(self, save=True):
        ...
        gpd_quantiles = stats.genpareto.ppf(
            probabilities_qq,
            c=self.parameters[2],
            loc=self.parameters[0],
            scale=self.parameters[1],
        ...
        gpd_probs = stats.genpareto.cdf(
            self.pot_data_sorted,
            c=self.parameters[2],
            loc=self.parameters[0],
            scale=self.parameters[1],
        )
        ...
        if save:
            if self.folder:
                plt.savefig(
                    f"{self.folder}/DiagnosticPlots.png", dpi=300, bbox_inches="tight"
                )
            else:
                print("Warning: No folder path specified in config. Saving skipped.")
    ...
    def gpd_qqplot(self):
        ...
        """
        # Calcular cuantiles teóricos de la GPD ajustada
        probabilities = (np.arange(1, self.n_pot + 1)) / (
            self.n_pot + 1
        )  # Probabilidades de los cuantiles empíricos
        gpd_quantiles = stats.genpareto.ppf(
            probabilities,
    ...
    def gpd_ppplot(self):
        ...
        """
        # Calcular cuantiles teóricos de la GPD ajustada
        probabilities = (np.arange(1, self.n_pot + 1) - 0.5) / (
            self.n_pot + 1
        )  # Probabilidades de los cuantiles empíricos
        gpd_probs = stats.genpareto.cdf(
            self.pot_data_sorted,
    ...
    def _pot_correction(self):
        """
        Apply POT extreme-correction
        """
        # POT correction on historical data
        self.ecdf_pot_probs_hist = np.arange(1, self.n_year_peaks + 1) / (
            self.n_year_peaks + 1
        )  # ECDF
        self.runif_pot_probs_hist = np.sort(
            np.random.uniform(low=0, high=1, size=self.n_year_peaks)
        )  # Random Uniform
        ...
        self.max_data_corrected = q_pot(
            self.runif_pot_probs_hist, self.parameters, self.poiss_parameter
        )
        ...
        if self.n_year_peaks > 1:
            ...
            aux_pit_corrected = np.interp(
                ...
                np.append(
                    min(self.pit_data), self.max_data_sorted
                ),  # x-coords of data points
                np.append(
                    min(self.pit_data), self.max_data_corrected
    ...
    def _annmax_correction(self):
        """
        Apply AM extreme-correction
        """
        # AnnualMaxima correction on historical data
        self.ecdf_annmax_probs_hist = np.arange(1, self.n_year_peaks + 1) / (
            self.n_year_peaks + 1
        )  # ECDF
        self.runif_annmax_probs_hist = np.sort(
            np.random.uniform(low=0, high=1, size=self.n_year_peaks)
        )  # Random Uniform
    ...
    def _pot_ci_return_period_bootstrap(self, B: int = 1000):
        ...
        boot_samples = np.random.choice(
            self.pot_data, size=(B, self.n_pot), replace=True
        )
        ...
        for i in range(B):
            shape, _, scale = stats.genpareto.fit(
                boot_samples[i] - self.opt_threshold, floc=0
            )
            boot_params[i] = [self.opt_threshold, scale, shape]

        # Vectorized return period computation
        return_periods = np.array(
            [q_pot(probs_ci, params, self.poiss_parameter) for params in boot_params]
        )

        self.lower_pot_ci_return = np.quantile(
            return_periods, (1 - self.conf) / 2, axis=0
        )
        self.upper_pot_ci_return = np.quantile(
            return_periods, 1 - (1 - self.conf) / 2, axis=0
        )
        ...
        for idx, year in enumerate(self.ci_T_years):
            ...
            try:
                ...
                try:
                    ...
                    print(
                        f"Warning: Could not find confidence interval for return period {year}. Error: {str(e)}"
                    )
    ...
    def _pot_return_period_plot(
        ...
        T_pt_corrected_hist = (
            1.0 / (1.0 - ecdf_pt_probs_corrected_hist) / self.freq
        )  # / n_return_period[wt]
        ...
        self.x_vals_gpd_poiss_hist = q_pot(
            1 - 1 / self.ci_T_years, self.parameters, self.poiss_parameter
        )
        ...
        self.T_annmax_corrected_hist = 1.0 / (
            1.0 - ecdf_annmax_probs_corrected_hist
        )  # *(40/self.n_peaks)
        ...
        if show_corrected:
            ...
            ax.semilogx(
                ...
                marker="o",
                markersize=3,
                label=r"Corrected Annual Maxima",
            )
        ...
        ax.set_ylim(
            0,
            np.max(
                np.concatenate([self.x_vals_gpd_poiss_hist, self.max_data_sorted]) + 2
            ),
        )
        # ax.set_ylim(0, 100)
    ...
    def _annmax_return_period_plot(
        ...
        self.x_vals_gev_hist = np.linspace(
            self.max_data_corrected[0], self.max_data_corrected[-1], 1000
        )
        ...
        # POT (uncorrected)
        self.T_pot_hist = 1.0 / (
            1.0 - self.ecdf_annmax_probs_hist
        )  # *(40/self.n_peaks)
        ...
        hess = ndt.Hessian(
            aux_fun, step=1e-5
        )  # Añado el step para que no de problemas de inestabilidad
        hessians_gev = hess(
        ...
        stdDq_gev = np.sqrt(
            np.sum((dqgev.T @ invI0_gev) * dqgev.T, axis=1)
        )  # Es lo mismo
        self.stdup_gev = self.x_vals_gev_hist + stdDq_gev * stats.norm.ppf(
            1 - (1 - self.conf) / 2, 0, 1
        )
        self.stdlo_gev = self.x_vals_gev_hist - stdDq_gev * stats.norm.ppf(
            1 - (1 - self.conf) / 2, 0, 1
        )
        ...
        if show_corrected:
            ax.semilogx(
                T_pt_corrected_hist,
                np.sort(self.pit_data_corrected),
                linewidth=0,
                marker="o",
                markersize=3,
                label="Corrected Daily Data",
            )
            ax.semilogx(
                self.T_ev_corrected_hist,
                stats.genextreme.ppf(
                    self.ecdf_annmax_probs_hist,
                    self.parameters[2],
                    loc=self.parameters[0],
        ...
        ax.semilogx(
            self.T_gev_fitted, self.stdlo_gev, color="tab:gray", linestyle="dotted"
        )
        ...
        if self.folder is not None:
            plt.savefig(
                f"{self.folder}/Historical_ReturnPeriod.png",
                dpi=300,
                bbox_inches="tight",
            )
        # plt.close(fig)
    ...
    def apply_sim_correction(self, quantile_to_correct: float = 0.8, random_state=None):
        """
        Apply extreme correction procedure in sampled data
        """
        if random_state is not None:
            np.random.seed(random_state)
        self.quantile_to_correct = quantile_to_correct
    ...
    def _ecdf_hist_data(self, q):
        return ECDF(self.pit_data)(q)

    def _ecdf_sim_data(self, q):
        return ECDF(self.sim_pit_data)(q)
    ...
    def _pot_correction_sim(self):
        ...
        for idx, value in enumerate(self.sim_max_data_sorted):
            if value == 0 or self._ecdf_sim_data(value) < self._ecdf_hist_data(
                self.opt_threshold
            ):
                self.am_index_0 += 1
            else:
        ...
        # Correct AM if it is non-zero and above the threshold
        self.sim_max_data_corrected = np.zeros(self.n_sim_year_peaks - self.am_index_0)

        # self.ecdf_max_probs_sim = np.arange(1, self.n_sim_year_peaks + 1) / (self.n_sim_year_peaks + 1)   # ECDF
        self.runif_max_probs_sim = np.sort(
            np.random.uniform(
                low=0, high=1, size=self.n_sim_year_peaks - self.am_index_0
            )
        )  # Random Uniform
        self.sim_max_data_corrected = q_pot(
            self.runif_max_probs_sim, self.parameters, self.poiss_parameter
        )  # Corrected POT

        sim_aux_pit_corrected = self.sim_pit_data.copy()  # Copy original array
        if self.n_sim_year_peaks > 1:
            q_threshold = np.quantile(self.sim_pit_data, self.quantile_to_correct)

            # Interpolate the rest of values
            sim_aux_pit_corrected = np.interp(
                self.sim_pit_data,  # x-coords to interpolate
                np.append(
                    [min(self.sim_pit_data), q_threshold], self.sim_max_data_sorted[self.am_index_0 :]
                ),  # x-coords of data points
                np.append(
                    [min(self.sim_pit_data), q_threshold], self.sim_max_data_corrected
                ),  # y-coords of data points
            )
    ...
    def _annmax_correction_sim(self):
        ...
        self.ecdf_annmax_probs_sim = np.arange(1, self.n_sim_year_peaks + 1) / (
            self.n_sim_year_peaks + 1
        )  # ECDF
        self.runif_annmax_probs_sim = np.sort(
            np.random.uniform(low=0, high=1, size=self.n_sim_year_peaks)
        )  # Random Uniform
        # Correct Annual Maxima using the fitted GEV
        self.sim_max_data_corrected = stats.genextreme.ppf(
            self.runif_annmax_probs_sim,
            self.parameters[2],
            loc=self.parameters[0],
            scale=self.parameters[1],
        )
        ...
        if self.n_sim_year_peaks > 1:
            ...
            sim_aux_pit_corrected = np.interp(
                self.sim_pit_data,  # x-coords to interpolate
                np.append(
                    min(self.sim_pit_data), self.sim_max_data_sorted
                ),  # x-coords of data points
                np.append(
                    min(self.sim_pit_data), self.sim_max_data_corrected
                ),  # y-coords of data points
            )
    ...
    def sim_return_period_plot(self, show_corrected=True, show_uncorrected=True):
        ...
        if self.method == "pot":
            fig, ax = self._pot_sim_return_period_plot(
                show_corrected=show_corrected, show_uncorrected=show_uncorrected
            )
        elif self.method == "am":
    ...
    def _pot_sim_return_period_plot(self, show_corrected=True, show_uncorrected=True):
        """
        ...
        ###### Daily Data ######
        # Daily corrected data
        ecdf_pt_probs_corrected_sim = np.arange(1, self.n_sim_pit + 1) / (
            self.n_sim_pit + 1
        )
        T_pt_corrected_sim = (
            1.0 / (1.0 - ecdf_pt_probs_corrected_sim) / self.freq
        )  # / n_return_period[wt]
        ...
        # GPD-Poisson fit over a grid of x-values
        self.x_vals_gpd_poiss_sim = np.linspace(
            self.sim_max_data_corrected_sorted[0],
            self.sim_max_data_corrected_sorted[-1],
            1000,
        )
        # Return period from GPD-Poisson fit
        gpd_poiss_probs_fitted_sim = cdf_pot(
            self.x_vals_gpd_poiss_sim,
            self.parameters[0],
            self.poiss_parameter,
            self.parameters[1],
            self.parameters[2],
        ...
        self.x_vals_gpd_poiss_sim = q_pot(
            1 - 1 / self.ci_T_years, self.parameters, self.poiss_parameter
        )
        ...
        self.T_annmax_corrected_sim = 1.0 / (
            1.0 - ecdf_annmax_probs_corrected_sim
        )  # *(40/self.n_peaks)
        ...
        if show_corrected:
            ...
            ax.semilogx(
                self.T_annmax_sim,
                np.sort(self.sim_pit_data_corrected[self.sim_max_idx]),
                color="red",
                linewidth=0,
                marker="^",
                markersize=5,
        ...
        if self.folder is not None:
            plt.savefig(
                f"{self.folder}/Simulation_ReturnPeriod{label}.png",
                dpi=300,
                bbox_inches="tight",
            )
        # plt.close(fig)
    ...
    def _annmax_sim_return_period_plot(
        self, show_corrected=True, show_uncorrected=True
    ):
        """
        ...
        x_vals_gev_sim = np.linspace(
            self.sim_max_data_corrected[0], self.sim_max_data_corrected[-1], 1000
        )
        ...
        T_gev_fitted = 1.0 / (
            1.0 - gev_probs_fitted
        )  # *(40/len(max_data_hist[wt]))#(10000/n_peaks)

        # Corrected peaks: re-check CDF and return periods
        ecdf_annmax_probs_corrected_sim = stats.genextreme.cdf(
            stats.genextreme.ppf(
                self.ecdf_annmax_probs_sim,
                self.parameters[2],
                loc=self.parameters[0],
                scale=self.parameters[1],
            ),
        ...
        # POT (uncorrected)
        T_pot_sim = 1.0 / (
            1.0 - self.ecdf_annmax_probs_sim
        )  # *(40/len(max_data_hist[wt]))#(10000/n_peaks)
        ...
        hessians_gev_sim = hess(
            [self.parameters[0], self.parameters[1], self.parameters[2]]
        )
        invI0_gev_sim = np.linalg.inv(hessians_gev_sim)

        stdDq_gev_sim = np.sqrt(
            np.sum((dqgev_sim.T @ invI0_gev_sim) * dqgev_sim.T, axis=1)
        )  # Es lo mismo
        stdup_gev_sim = x_vals_gev_sim + stdDq_gev_sim * stats.norm.ppf(
            1 - (1 - self.conf) / 2, 0, 1
        )
        stdlo_gev_sim = x_vals_gev_sim - stdDq_gev_sim * stats.norm.ppf(
            1 - (1 - self.conf) / 2, 0, 1
        )
        ...
        if show_corrected:
            ax.semilogx(
                T_pt_corrected_sim,
                np.sort(self.sim_pit_data_corrected),
                color="tab:blue",
                linewidth=0,
                marker="o",
                markersize=10,
            ...
            ax.semilogx(
                ...
                marker="o",
                markersize=5,
                label=f"Corrected Annual Maxima",
            )
            label = label + "_Corr"
    ...
    def interval_return_period_plot(self, alpha=0.2):
        """DEPRECATED"""
    ...
    def _pot_interval_return_period_plot(self, alpha=0.2):
        """
        DEPRECATED
        ...
        for i_year in range(self.n_year_intervals):
            # Corrected
            new_max_idx_sim_int[i_year] = self.data_sim[self.var].index.get_indexer(
                self.sim_max_data_idx_intervals[i_year]
            )
            annual_maxima_corr_sim_int[i_year] = self.sim_pit_data_corrected[
                new_max_idx_sim_int[i_year]
            ]
            ecdf_annual_maxima_sim_int[i_year] = np.arange(
                1, len(annual_maxima_corr_sim_int[i_year]) + 1
            ) / (len(annual_maxima_corr_sim_int[i_year]) + 1)
            T_ecdf_annual_maxima_sim_int[i_year] = (
                1
                / (1 - ecdf_annual_maxima_sim_int[i_year])
                * (self.n_year_peaks / len(self.sim_max_data_idx_intervals[i_year]))
            )
            # No corrected
            annual_maxima_nocorr_sim_int[i_year] = self.data_sim[self.var][
                self.sim_max_data_idx_intervals[i_year]
            ].values
            ecdf_annual_maxima_nocorr_sim_int[i_year] = np.arange(
                1, len(annual_maxima_nocorr_sim_int[i_year]) + 1
            ) / (len(annual_maxima_nocorr_sim_int[i_year]) + 1)
            T_ecdf_annual_maxima_nocorr_sim_int[i_year] = (
                1
                / (1 - ecdf_annual_maxima_nocorr_sim_int[i_year])
                * (self.n_year_peaks / len(self.sim_max_data_idx_intervals[i_year]))
            )
        ...
        ax1.semilogx(
            T_ecdf_annual_maxima_nocorr_sim_int[0],
            np.sort(annual_maxima_nocorr_sim_int[0]),
            color="tab:gray",
            alpha=alpha,
            label=f"No Corrected Simulated Data by {self.n_year_peaks} Years",
        )
        for i_year in range(1, self.n_year_intervals):
            max1.append(np.max(annual_maxima_nocorr_sim_int[i_year]))
            ax1.semilogx(
                T_ecdf_annual_maxima_nocorr_sim_int[i_year],
                np.sort(annual_maxima_nocorr_sim_int[i_year]),
                color="tab:gray",
                alpha=alpha,
            )
        ...
        ax2.semilogx(
            T_ecdf_annual_maxima_nocorr_sim_int[0],
            np.sort(annual_maxima_corr_sim_int[0]),
            color="tab:gray",
            alpha=alpha,
            label=f"Corrected Simulated Data by {self.n_year_peaks} Years",
        )
        for i_year in range(1, self.n_year_intervals):
            max2.append(np.max(annual_maxima_corr_sim_int[i_year]))
            ax2.semilogx(
                T_ecdf_annual_maxima_nocorr_sim_int[i_year],
                np.sort(annual_maxima_corr_sim_int[i_year]),
                color="tab:gray",
                alpha=alpha,
            )
        ...
        ax1.semilogx(
            self.T_gpd_poiss_fitted,
            np.sort(self.x_vals_gpd_poiss_hist),
            color="red",
            linestyle="dashed",
            label=f"Adjusted GPD-Poisson",
        )
        ax1.semilogx(
            self.T_annmax_corrected_hist,
            self.max_data_sorted,
            color="tab:blue",
            linewidth=0,
            marker="o",
        ...
        if self.folder is not None:
            plt.savefig(
                f"{self.folder}/ComparativeIntervals_ReturnPeriod.png",
                dpi=300,
                bbox_inches="tight",
            )
        plt.close(fig)

    def _annmax_interval_return_period_plot(self, alpha=0.2):
        """
        DEPRECATED
        ...
        ax1.semilogx(
            ...
            color="black",
            linestyle="dotted",
            label=f"{self.conf} Conf Int",
        )
        ax1.semilogx(
            self.T_gev_fitted, self.stdlo_gev, color="black", linestyle="dotted"
        )
